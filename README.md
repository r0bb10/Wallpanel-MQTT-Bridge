...
**Edit sudoers** (replace `r88t` with your username):
```bash
sudo visudo
```
...
Add these lines:
```
r88t ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart wallpanel-mqtt-bridge
r88t ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload wallpanel-mqtt-bridge
r88t ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart greetd
r88t ALL=(ALL) NOPASSWD: /sbin/reboot
```
...
- Commands run as the service user (e.g., `r88t`)
